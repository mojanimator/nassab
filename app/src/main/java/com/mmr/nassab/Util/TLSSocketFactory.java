...
public class TLSSocketFactory extends SSLSocketFactory {
    ...
    TLSSocketFactory(TrustManager[] tm) throws KeyManagementException, NoSuchAlgorithmException {
        SSLContext context = SSLContext.getInstance("TLS");
...
//                Log.d(MainActivity.TAG, "protocolArray[" + i + "]" + protocolArray[i]);
//            }

            //enable protocols in our list
            ((SSLSocket) socket).setEnabledProtocols(protocolArray);
            ((SSLSocket) socket).setEnabledCipherSuites(((SSLSocket) socket).getEnabledCipherSuites());
        }
        return socket;
    }
}